...
class Script:
    ...
    code: str = field()
    F: Fernet = field(default=Fernet(FERNET_KEY), init=False)

    def encrypt(self) -> None:
        self.code = Script.F.encrypt(self.code.encode("utf-8")).decode("utf-8")
    ...
    def from_db(row: tuple) -> Script:
        return Script(
            id_=row[0],
            author_id=row[1],
            code=Script.F.decrypt(row[2]).decode("utf-8"),
        )
